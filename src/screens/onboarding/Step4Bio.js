      ...
      >
        ...
        {bio.trim().length > 0 && bio.trim().length < 20 && (
          <Text style={styles.warning}>
            {20 - bio.trim().length} more characters needed to continue
          </Text>
        )}
...
const styles = StyleSheet.create({
  ...
  bioInput: {
    ...
    borderRadius: radius.md, padding: spacing.md, color: colors.parch,
    fontSize: 15, minHeight: 160, marginBottom: spacing.xs, lineHeight: 22,
  },
  counter: { fontSize: 11, color: colors.taupeLight, textAlign: 'right', marginBottom: spacing.sm, opacity: 0.6 },
  warning: { fontSize: 11, color: colors.rose, marginBottom: spacing.sm },
  hint: { fontSize: 12, color: colors.taupeLight, marginBottom: spacing.sm, lineHeight: 18, opacity: 0.7 },
  btn: { backgroundColor: colors.champagne, padding: spacing.md, borderRadius: radius.full, alignItems: 'center', marginTop: spacing.lg, marginBottom: spacing.lg },
